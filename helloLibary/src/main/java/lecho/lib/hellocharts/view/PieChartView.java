...
/**
 * PieChart is a little different than others charts. It doesn't have axes. It
 * doesn't support viewport so changing viewport wont work. Instead it support
 * "Circle Oval". Pinch-to-Zoom and double tap zoom wont work either. Instead of
 * scroll there is chart rotation if isChartRotationEnabled is set to true.
 * PieChart looks the best when it has the same width and height, drawing chart
 * on rectangle with proportions other than 1:1 will left some empty spaces.
 * 
 * @author Leszek Wach
 */
public class PieChartView extends AbstractChartView implements
	...
	public void callTouchListener() {
		...
		if (selectedValue.isSet()) {
			SliceValue sliceValue = data.getValues().get(
	...
	public void setOnValueTouchListener(
			PieChartOnValueSelectListener touchListener) {
	...
	/**
	 * Use this to change pie chart area. Because by default CircleOval is
	 * calculated onSizeChanged() you must call this method after size of
	 * PieChartView is calculated. In most cases it will probably be easier to
	 * use {@link #setCircleFillRatio(float)} to change chart area or just use
	 * view padding.
	 */
	...
	/**
	 * Returns pie chart rotation, 0 rotation means that 0 degrees is at 3
	 * o'clock. Don't confuse with {@link View#getRotation()}.
	 * 
	 * @return
	 */
	...
	public void setChartRotation(int rotation, boolean isAnimated) {
		if (isAnimated) {
			...
			rotationAnimator.startAnimation(
					pieChartRenderer.getChartRotation(), rotation);
		} else {
	 ...
	 * Rotating programmatically will still work.
	 * 
	 * @param isRotationEnabled
	 */
	public void setChartRotationEnabled(boolean isRotationEnabled) {
		if (touchHandler instanceof PieChartTouchHandler) {
			((PieChartTouchHandler) touchHandler)
					.setRotationEnabled(isRotationEnabled);
		}
	}

	/**
	 * Returns SliceValue that is under given angle, selectedValue (if not null)
	 * will be hold slice index.
	 */
	...
	/**
	 * Set how much of view area should be taken by chart circle. Value should
	 * be between 0 and 1. Default is 1 so circle will have radius equals
