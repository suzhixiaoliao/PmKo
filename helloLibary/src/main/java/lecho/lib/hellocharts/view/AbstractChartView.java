...
public abstract class AbstractChartView extends View implements Chart {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (isInteractive) {
			setMeasuredDimension(widthMeasureSpec, (dm.heightPixels / 10) * 8);
		} else {
			setMeasuredDimension(widthMeasureSpec,
					(int) (dm.heightPixels / 4) * 1);
		}*/
	...
	protected void onSizeChanged(int width, int height, int oldWidth,
		...
		chartComputator.setContentRect(getWidth(), getHeight(),
	...
	@Override
	public void setViewportAnimationListener(
			ChartAnimationListener animationListener) {
		viewportAnimator.setChartAnimationListener(animationListener);
	}

	@Override
	public void setViewportChangeListener(
	...
	private Viewport computeScrollViewport(float x, float y) {
		...
		if (maxViewport.contains(x, y)) {
			...
			left = Math.max(maxViewport.left,
					Math.min(left, maxViewport.right - width));
			top = Math.max(maxViewport.bottom + height,
					Math.min(top, maxViewport.top));
	...
	public void setCurrentViewportWithAnimation(Viewport targetViewport) {
		if (null != targetViewport) {
			...
			viewportAnimator.startAnimation(getCurrentViewport(),
					targetViewport);
		}
		ViewCompat.postInvalidateOnAnimation(this);
	}
	...
	/**
	 * You should call this method in derived classes, most likely from
	 * constructor if you changed chart/axis renderer, touch handler or chart
	 * computator
	 */
