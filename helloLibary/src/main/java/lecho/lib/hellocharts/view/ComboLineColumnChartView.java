...
/**
 * ComboChart, supports ColumnChart combined with LineChart. Lines are always
 * drawn on top.
 * 
 * @author Leszek Wach
 */
public class ComboLineColumnChartView extends AbstractChartView implements
		ComboLineColumnChartDataProvider {
	...
	public ComboLineColumnChartView(Context context, AttributeSet attrs,
		...
		setChartRenderer(new ComboLineColumnChartRenderer(context, this,
				columnChartDataProvider, lineChartDataProvider));
		setComboLineColumnChartData(ComboLineColumnChartData
				.generateDummyData());
	}
	...
	public void callTouchListener() {
		...
		if (selectedValue.isSet()) {
			...
			if (SelectedValueType.COLUMN.equals(selectedValue.getType())) {
				...
				SubcolumnValue value = data.getColumnChartData().getColumns()
				...
				PointValue value = data.getLineChartData().getLines()
						.get(selectedValue.getFirstIndex()).getValues()
						.get(selectedValue.getSecondIndex());
	...
	public void setOnValueTouchListener(
			ComboLineColumnChartOnValueSelectListener touchListener) {
		if (null != touchListener) {
			this.onValueTouchListener = touchListener;
		}
	}

	public void setColumnChartRenderer(Context context,
			ColumnChartRenderer columnChartRenderer) {
		setChartRenderer(new ComboLineColumnChartRenderer(context, this,
				columnChartRenderer, lineChartDataProvider));
	}

	public void setLineChartRenderer(Context context,
			LineChartRenderer lineChartRenderer) {
		setChartRenderer(new ComboLineColumnChartRenderer(context, this,
				columnChartDataProvider, lineChartRenderer));
	}
	...
	private class ComboColumnChartDataProvider implements
