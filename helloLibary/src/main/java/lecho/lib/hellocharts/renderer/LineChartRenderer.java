...
public class LineChartRenderer extends AbstractChartRenderer {
	...
	private void drawPath(Canvas canvas, final Line line) {
		...
		for (int index = 0; index < line.getValues().size(); index++) {
			currentX = line.getValues().get(index).getX();
			currentY = line.getValues().get(index).getY();
			if (index == 0) {
				Log.i("current", "currentX  = = = " + currentX);
				Log.i("current", "currentY  = = = " + currentY);
			}
			if (!Float.isNaN(currentY)) {
				currentX = computator.computeRawX(currentX);
				currentY = computator.computeRawY(currentY);
				path.moveTo(currentX, currentY);
				if ((index + 1) < line.getValues().size()) {
					nextX = line.getValues().get(index + 1).getX();
					nextY = line.getValues().get(index + 1).getY();
					if (!Float.isNaN(nextY)) {
						nextX = computator.computeRawX(nextX);
						nextY = computator.computeRawY(nextY);
