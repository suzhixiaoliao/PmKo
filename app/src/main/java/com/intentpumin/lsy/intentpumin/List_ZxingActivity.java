...
import com.intentpumin.lsy.intentpumin.tools.RusultData;
...
public class List_ZxingActivity extends AppCompatActivity {
    ...
    private ZxingTaskGridAdapter adapter;
    // TODO: 2016/6/21
    private ZxingDataListAdapter dataadapter;
    private List<item_task> mtast;
    private List<DataTask> mstat;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.layout_zxing_list_main);
    ...
    private void initAction() {
        mtasklist.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                String finished = mtast.get(position).getFinished();
                if (finished.equals("N")) {
                    mtast.get(position).setFinished("Y");
                } else if (finished.equals("Y")) {
                    mtast.get(position).setFinished("N");
                }
    ...
    private void init() {
        ...
        mtasklist.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                item_task task = (item_task) adapter.getItem(position);
                Intent inter = getIntent();
                inter.setClass(List_ZxingActivity.this, TastComActivity.class);
        ...
        adapter = new ZxingTaskGridAdapter(this, mtast);
        ...
        requestData(result);
        mstat = new ArrayList<>();
        dataadapter = new ZxingDataListAdapter(this, mstat);
    ...
    private void requestvalue() {
        ...
        //进行解析
        Type type = new TypeToken<RusultItem>() {
        }.getType();
        Gson gson = new Gson();
        RusultItem b = gson.fromJson(tastList, type);
      ...
      /*  if (mlogin != null && !TextUtils.isEmpty(mlogin.getPhoneno())) {
        ...
        HttpUtil.getInstance().post(MainLogic.GET_STAT, params, new StringHttpRequestCallback() {
            ...
            protected void onSuccess(String s) {
                ...
                RusultData resulut = null;

                try {
                    if (!TextUtils.isEmpty(s)) {
                        Gson gson = new Gson();

                        resulut = gson.fromJson(s, RusultData.class);//将JSON数据转成Result对象
...
//        String eqpt_id = b.getData().getItems().get(0).getEqpt_id();
        ...
        HttpUtil.getInstance().post(MainLogic.GET_TASK, params, new StringHttpRequestCallback() {
            ...
            protected void onSuccess(String s) {
                ...
                try {
                    if (!TextUtils.isEmpty(s)) {
                        Gson gson = new Gson();
                        result = gson.fromJson(s, Rusult.class);//将JSON数据转成Result对象
                        // TODO: 2016/6/22 保存Sp数据
                ...
                mtast = result.getData().getItems();
                adapter.setItems(mtast);
                adapter.notifyDataSetChanged();


                LogUtils.LOGD("login4", mtast.toString());
            }
    ...
    private void postStat(DataTask task) {
        ...
        //进行解析
        Type type = new TypeToken<RusultData>() {
        }.getType();
        Gson gson = new Gson();
        RusultData b = gson.fromJson(resulut, type);
        if (mstat != null && mstat.size() > 0) {
            for (int i = 0; i < mstat.size(); i++) {
                final int j = i;
                DataTask item = mstat.get(i);
//                if (item.getFinished().equals("Y1")) {
                ...
                HttpUtil.getInstance().post(MainLogic.SET_STAT, params, new StringHttpRequestCallback() {
                    ...
                    protected void onSuccess(String s) {
                        ...
                        Toast.makeText(List_ZxingActivity.this, "保存数据成功", Toast.LENGTH_SHORT).show();
                    }
...
//                }
    ...
    private void postTask() {
        ...
        //进行解析
        Type type = new TypeToken<Rusult>() {
        }.getType();
        Gson gson = new Gson();
        Rusult b = gson.fromJson(result, type);
        if (mtast != null && mtast.size() > 0) {
            for (int i = 0; i < mtast.size(); i++) {
                final int j = i;
                item_task item = mtast.get(i);
               ...
               // params.addFormDataPart("finished", b.getData().getItems().get(i).getFinished());
                ...
                HttpUtil.getInstance().post(MainLogic.SET_TASK, params, new StringHttpRequestCallback() {
                    ...
                    protected void onSuccess(String s) {
                        System.out.println("onSuccess=======" + s);
                        if (mtast.get(j).getFinished().equals("Y1")) {
                            mtast.get(j).setFinished("Y");
                            LogUtils.LOGD("login8", s.toString());
                        }
                        if (mtast.get(j).getFinished().equals("N1")) {
                            mtast.get(j).setFinished("N");
                            LogUtils.LOGD("login9", s.toString());
                        }

                        System.out.println(s);
                        LogUtils.LOGD("login6", s.toString());
                        Toast.makeText(List_ZxingActivity.this, "保存任务成功", Toast.LENGTH_SHORT).show();
